...
class ChessBoard:
    ...
    def draw_squares(self, win, size, firstBoard):
        """
        ...
        if firstBoard:
            ...
            for row in range(border, rows + border):
                for col in range(row % 2 + border - 1, cols + border - 1, 2):
                    ...
                    listOfGraySquares.append((row * SQUARE_SIZE, col * SQUARE_SIZE))
                ...
                for col in range((row + 1) % 2 + border - 1, cols + border - 1, 2):   
                    ...
                    listOfBlueSquares.append((row * SQUARE_SIZE, col * SQUARE_SIZE))
